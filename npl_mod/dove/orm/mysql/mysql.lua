...
function mysql:init(config)
	...
	config.username = config.username or "root"
...
function mysql:get_where_str(t)
	...
	for key, value in pairs(t) do
		...
		if pos ~= 1 then
			...
			if not is_first then sql_str = sql_str .. " and " end
			is_first = false
	...
	if t[mysql.OR] then
		...
		if not is_first then sql_str = sql_str .. ")" end
	end
...
function mysql:count(q)
	...
	if not cur then return 0, err end
	...
	if #list == 1 then return tonumber(list[1]["count"]) end
...
function mysql:find(q)
	...
	local row = {}
	local cur, err = mysql:execute(sql_str)
	if not cur then return list, err end
...
function mysql:insert(q)
	...
	if ret == nil then return err end
...
function mysql:upsert(q, o)
	--q = self:_type_convert(q)
	--o = self:_type_convert(o)
	if self:find_one(q) then return self:update(q,o) end
